...
class FPDF
{
    ...
    protected $utf8;               // indicates whether UTF-8 encoding is used
    protected $display_mode;       // display mode
    ...
    *******************************************************************************/
    ...
    protected function _out($s)
    {
        ...
        elseif($this->state==1)
            $this->_put($s);
        elseif($this->state==0)
            $this->Error('No document started yet');
        elseif($this->state==3)
            $this->Error('The document is closed');
    }
    ...
    {
        $this->_put('/Producer '.$this->_textstring('FPDF '.self::VERSION));
        if(!empty($this->title))
    ...
    // For internal use
    public function SetDash($dash_on, $dash_off)
    {
        if($dash_on==0 && $dash_off==0)
            $s = "[] 0 d";
        else
            $s = "[$dash_on $dash_off] 0 d";
        $this->_out($s);
    }
